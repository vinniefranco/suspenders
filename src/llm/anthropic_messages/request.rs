...
fn wire_block(block: &ContentBlock) -> Value {
    match block {
        ...
        // All non-ToolResult variants derive Serialize and contain only
        // JSON-representable types, so serialization is infallible here.
        other => serde_json::to_value(other)
            .expect("ContentBlock derives Serialize and never contains non-JSON types"),
    }
}
...
#[cfg(test)]
#[path = "../../../tests/unit/llm/anthropic_messages/request.rs"]
mod tests;
