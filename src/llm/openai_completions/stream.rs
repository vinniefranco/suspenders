...
/// Folds a sequence of parsed SSE events into a [`Response`] by driving the
/// production [`StreamState`] in batch. Test-only: the live `complete` path
/// drives `handle_event`/`finalize` incrementally to snapshot mid-stream deltas.
#[cfg(test)]
...
#[cfg(test)]
#[path = "../../../tests/unit/llm/openai_completions/stream.rs"]
mod tests;
