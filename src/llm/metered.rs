...
/// A cost accumulation of zero: the threshold below which the sink is silent
/// (zero-usage calls and unpriced local models do not move the total).
const ZERO_COST: f64 = 0.0;
...
impl Llm for Metered {
    async fn complete(
        ...
        if let Some(cost) = model.cost(&response.usage)
            && cost.total > ZERO_COST
        {
            let total = {
                // The lock is never poisoned: the only mutation is `+= cost`,
                // which cannot panic, so no thread can poison the Mutex.
                let mut total = self.total.lock().expect("total lock is never poisoned");
                *total += cost.total;
                *total
            };
            (self.on_total)(total);
...
#[cfg(test)]
#[path = "../../tests/unit/llm/metered.rs"]
mod tests;
