...
/// Counts consecutive `\` characters immediately before `before` in `chars`.
/// An odd count means the character at index `before` is backslash-escaped.
/// Used by the command-validation and AT-completion logic, which share the same
/// backslash-parity rule.
pub fn preceding_backslash_count(chars: &[char], before: usize) -> usize {
    chars[..before]
        .iter()
        .rev()
        .take_while(|&&c| c == '\\')
