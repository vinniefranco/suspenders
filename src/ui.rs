...
async fn run_loop(
    ...
    draw_previewed(
        terminal,
        screen.as_ref().expect("screen slot always Some at startup"),
        &mut cache,
        DrawInputs {
            conn: &conn,
            anim,
    ...
    screen = Some(
        dispatch(
            terminal,
            screen.take().expect("screen slot always Some at startup"),
            Vec::new(),
            &mut Adapter {
                ctx: &ctx,
                state: &mut state,
    ...
    loop {
        tokio::select! {
            ...
            maybe_input = input.next() => {
                let dirty = match on_input_batch(
                    &mut LoopFrame {
                        terminal,
                        screen: &mut screen,
                        cache: &mut cache,
                ...
                .await?
                {
                    BatchOutcome::Quit => return Ok(()),
                    BatchOutcome::Continue { dirty } => dirty,
                };
                // A `/model` pick commits as a key press in this batch: refresh.
                refresh_active_model(&agent, &mut conn, session).await;
                if !dirty {
                    continue;
                }
            }

            // Agent events: a run result, a lag resync, or agent-down.
            recv = events.recv() => {
                match recv {
                    Ok(event) => {
                        fold_event(
                    ...
                    Err(RecvError::Closed) => {
                        return on_agent_down(
                            terminal,
                            // moved: loop ends after this return
                            LoopEnd {
                                screen,
                                conn,
                                anim,
            ...
            Some(event) = selector_rx.recv() => {
                ...
                .await?;
                // The fetch never changes the model, but a pick could have raced in.
                refresh_active_model(&agent, &mut conn, session).await;
            }
        }
        ...
        draw_previewed(
            ...
            screen
                .as_ref()
                .expect("screen slot always Some after select arm"),
            &mut cache,
            DrawInputs {
                conn: &conn,
                anim,
...
// ---------------------------------------------------------------------------
// run_loop select-arm handlers (extracted to shorten the orchestrator)
// ---------------------------------------------------------------------------

/// Refresh the status-bar Active Model and its endpoint from the Agent, off the
/// tick (a cheap in-process actor query, ADR-0017/0033). The endpoint follows
/// the scoped id's Provider (ADR-0037) so a cross-Provider pick never leaves a
/// stale base_url up. A `None` reply means the Agent died mid-batch (racing
/// shutdown): keep the last-shown model so the bar never blanks, and let the
/// Closed arm drive the graceful agent-down path.
async fn refresh_active_model(
    agent: &AgentHandle,
    conn: &mut components::ConnectionFacts,
    session: &Session,
) {
    if let Some(model) = agent.active_model().await {
        conn.model = model;
        conn.base_url = provider_base_url(session, &conn.model);
    }
}

/// The mutable environment the borrowing run-loop handlers fold against: the
/// terminal, the Option<Screen> slot (taken and replaced per fold), the render
/// cache the commit seam blits through, the adapter-mutable state, and the
/// immutable adapter ctx. These five travel together to every borrowing handler,
/// so they ride as ONE carrier instead of five positional params. Constructed
/// per select-arm as a temporary, so it never holds screen/cache/state borrowed
/// across the wider select! (same discipline as `Adapter`).
struct LoopFrame<'a, B: Backend> {
...
/// The owned loop state handed to the agent-down exit path (the loop ends, so
/// these move rather than borrow).
struct LoopEnd {
...
/// Folds one [`Event`] (agent or adapter-injected) through the pure core and
/// dispatches the returned effects. The `screen` slot is always `Some` on
/// entry and `Some` on exit (the invariant the loop maintains).
async fn fold_event<B: Backend>(frame: &mut LoopFrame<'_, B>, event: Event) -> anyhow::Result<()> {
    let core = frame
        .screen
        .take()
        .expect("screen slot always Some when folding an event");
...
/// Handles the animation-tick select arm: expires the Approval timeout,
/// advances the anim counters, and repaints if a Run is active.
/// The caller issues `continue` after this returns to skip the trailing draw.
async fn on_tick<B: Backend>(
    frame: &mut LoopFrame<'_, B>,
    conn: &components::ConnectionFacts,
    anim: &mut components::Anim,
    started: std::time::Instant,
) -> anyhow::Result<()> {
    // Drive the Approval radio's digit-quick-select timeout (ADR-0049):
    // the pure core reads the elapsed-millis clock and, on a fired buffered
    // digit, resolves the Approval. A no-op when no Approval is open or
    // nothing is buffered (the 3-row radio never buffers).
    let core = frame
        .screen
        .take()
        .expect("screen slot always Some on tick");
    ...
    *frame.screen = Some(
    ...
    let s = frame
        .screen
        .as_ref()
        .expect("screen slot always Some after tick dispatch");
...
/// The outcome of one terminal-input batch: either the loop continues (carrying
/// whether the frame was dirtied and needs a redraw), or the user quit / the
/// input stream ended and the loop should exit.
enum BatchOutcome {
    /// Keep looping. `dirty` is true if any event changed the frame (redraw),
    /// false when only Release keys and no-ops arrived (skip the redraw).
    Continue { dirty: bool },
    /// A Ctrl-C/Ctrl-Q quit key, or the input stream ended: exit the run loop.
    Quit,
}

/// Handles the terminal-input select arm: coalesces a burst of ready events
/// into one fold+draw, returning a [`BatchOutcome`]. Returns an error only on a
/// propagated dispatch failure.
async fn on_input_batch<B: Backend, S>(
    frame: &mut LoopFrame<'_, B>,
    input: &mut S,
    first: Option<std::io::Result<CtEvent>>,
) -> anyhow::Result<BatchOutcome>
where
    S: futures_util::Stream<Item = std::io::Result<CtEvent>> + Unpin,
{
    ...
    loop {
        match pending {
            Some(Ok(CtEvent::Key(key_event))) => {
                ...
                if key_event.kind != KeyEventKind::Release {
                    ...
                    let core = frame
                        .screen
                        .take()
                        .expect("screen slot always Some when handling key");
            ...
            None => return Ok(BatchOutcome::Quit), // input stream ended
        }
        // Drain whatever is already buffered; the first
        // not-yet-ready poll ends the batch.
        match next_if_ready(input).await {
            Some(next) => pending = next,
            None => break,
        }
    }
    Ok(BatchOutcome::Continue { dirty })
}

/// Handles the `RecvError::Closed` (agent-down) case: applies `agent_down` to
/// the pure core, draws the frozen frame, and hands the input stream off to
/// [`drain_input`] for the quit-only tail. Consumes `screen`, `cache`, `conn`,
/// and `state` because they are all moved into the frozen frame after the Agent
/// is gone and the loop will not continue.
async fn on_agent_down<B: Backend, S>(
    terminal: &mut Terminal<B>,
    end: LoopEnd,
    input: S,
    ctx: &AdapterCtx<'_>,
    ...
    S: futures_util::Stream<Item = std::io::Result<CtEvent>> + Unpin,
{
    ...
    let core = screen
        .take()
        .expect("screen slot always Some on agent-down");
...
/// The per-draw dynamic inputs for [`draw_previewed`]: the connection facts,
/// animation state, and adapter state that together determine the frame's
/// theme and status bar content. Grouped so `draw_previewed` stays below the
/// SRP_PARAMS threshold (was 6 positional args).
pub(crate) struct DrawInputs<'a> {
    pub(crate) conn: &'a components::ConnectionFacts,
    pub(crate) anim: components::Anim,
    pub(crate) state: &'a AdapterState,
}
...
fn draw_previewed<B: Backend>(
    ...
    let DrawInputs { conn, anim, state } = inputs;
