...
impl ApprovalMode {
    ...
    /// `APPROVAL_MODES`): plan -> default -> auto-edit -> auto -> yolo -> plan.
    /// A total function with a hard wrap, so the cycle can never leave the set.
    pub fn cycle(self) -> ApprovalMode {
        // Mirrors qwen's `APPROVAL_MODES` array + `(i + 1) % len` rotation.
