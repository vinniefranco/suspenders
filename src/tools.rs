...
/// Extract an optional JSON string field from `input` by `key`. Returns `None`
/// for absent/null, `Ok(Some(s))` for a string value, and `Err(())` for any
/// other value type. Used by tool modules for optional string params (path, glob,
/// and similar).
pub(super) fn opt_str<'a>(input: &'a Value, key: &str) -> Result<Option<&'a str>, ()> {
    match input.get(key) {
