...
fn is_unescaped_space(chars: &[char], i: usize) -> bool {
    let backslashes = crate::tool::path::preceding_backslash_count(chars, i);
...
/// The fraction of the terminal height the Composer may claim: the transcript
/// keeps the larger share, so the composer box is bounded to `height / COMPOSER_HEIGHT_FRACTION`.
const COMPOSER_HEIGHT_FRACTION: usize = 3;
...
pub fn max_visible_rows(terminal_height: usize) -> usize {
    (terminal_height / COMPOSER_HEIGHT_FRACTION).clamp(1, MAX_ROWS)
}
...
#[cfg(test)]
#[path = "../../tests/unit/ui/composer.rs"]
mod tests;
