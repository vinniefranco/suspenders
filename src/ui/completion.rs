...
/// The recency score returned when a command has no use record.
const RECENCY_SCORE_NONE: f64 = 0.0;

/// The freshness ceiling (a use at `now`) and floor (clamped so an
/// older-than-decay use contributes nothing) of the linear recency decay.
const RECENCY_FRESHNESS_MAX: f64 = 1.0;
const RECENCY_FRESHNESS_MIN: f64 = 0.0;

/// Per-use recency weight (qwen `getRecentScore`): multiplied by both the
/// raw count AND the freshness term.
...
fn fuzzy_one(
    ...
    let window = if indices.is_empty() {
        ...
        // The `is_empty` guard above makes min/max infallible: `indices` has
        // at least one element, so the iterators always yield `Some`.
        let min = *indices.iter().min().expect("indices non-empty") as usize;
        let max = *indices.iter().max().expect("indices non-empty") as usize;
        Some((min, max + 1))
    };
    Some((score, window))
}
...
fn recent_score(recent: &dyn Fn(&str) -> Option<RecentUse>, name: &str, now: Millis) -> f64 {
    let Some(use_record) = recent(name) else {
        return RECENCY_SCORE_NONE;
    };
    let age = now.saturating_sub(use_record.used_at) as f64;
    let freshness =
        (RECENCY_FRESHNESS_MAX - age / RECENT_DECAY_MS as f64).max(RECENCY_FRESHNESS_MIN);
    use_record.count as f64 * RECENCY_WEIGHT + RECENCY_WEIGHT * freshness
}
...
#[cfg(test)]
#[path = "../../tests/unit/ui/completion.rs"]
mod tests;
