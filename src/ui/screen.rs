...
impl Screen {
    ...
    /// A representative populated Screen for the render tests: one user request
    /// whose run interleaves several Thinking passes, tool machinery, harness
    /// markers, and an answer with a code fence - the exact shape that exposed
    /// the fold / separator / blank-line bugs. No IO, no events; the transcript
    /// is authored directly. Test-only fixture (the live `--demo` binary uses
    /// [`Screen::demo_diffs`]).
    #[cfg(test)]
    pub fn demo() -> Self {
