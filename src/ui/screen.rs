...
pub enum Effect {
    ...
    PeekPending,
    /// Open the `/mcp` management dialog (ADR-0065 Phase E): the Composer opened
    /// the `McpDialog` overlay to a Loading state on commit and hands the adapter
    /// this effect to kick the async `Agent::mcp_views()` fetch. The adapter posts
    /// the result back as [`Event::McpDialogReady`](crate::event::Event::McpDialogReady)
...
impl Screen {
    ...
    fn apply_voice(mut self, event: Event) -> (Self, Vec<Effect>) {
        match event {
            ...
            // AUTHENTICATE dialog step (Phase E) will consume these directly; until
            // then the info line is where the copy-the-URL hint + the auth URL show.
