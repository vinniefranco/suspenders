//! The single semantic to ratatui Style/Color mapping (ADR-0008), coloured by the active Theme (ADR-0038): the one place display semantics become terminal colours.
//!
...
/// A semantic [`Style`] builder: the shared shape of every `*_style` slot
/// (`accent_style`, `primary_style`, …), named so tables that pair a role with
/// its slot ([`super::overlay::MCP_SPAN_STYLES`]) carry it as one entity rather
/// than respelling the bare `fn` pointer at each site.
pub(super) type StyleFn = fn(&Theme) -> Style;
