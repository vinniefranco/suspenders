//! The sticky "Current tasks" box (qwen StickyTodoList.tsx + todoSnapshot.ts, ADR-0048): a LIVE overlay derived from the Transcript's latest Todo item.
//!
...
pub(super) const STICKY_TODO_MAX_VISIBLE: usize = 5;
...
pub(super) fn sticky_status_priority(status: TodoStatus) -> u8 {
    match status {
...
pub(super) const STICKY_MIN_BODY_ROWS: usize = 1;
...
pub(super) const STICKY_GLYPH_COL: usize = 2;
...
pub(super) struct StickyColumns {
    visible: usize,
    hidden: usize,
    num_col: usize,
    content_col: usize,
...
pub(super) fn sticky_columns(ordered: &[(usize, &TodoItem)], inner: usize) -> StickyColumns {
...
pub(super) fn sticky_todos_lines(
    ...
    let body: Vec<Line<'static>> = sticky_rows(&ordered, cols, theme)
        .into_iter()
        .map(Line::from)
        .collect();
...
pub(super) fn sticky_number_col(shown: &[(usize, &TodoItem)]) -> usize {
    shown
        .iter()
        .map(|(orig, _)| number_label(orig + 1).chars().count())
        .max()
        .unwrap_or(2)
        + 1
}
...
pub(super) fn sticky_overflow_spans(
    hidden: usize,
    num_col: usize,
    content_col: usize,
    theme: &Theme,
...
pub(super) fn sticky_row_spans(
    ...
    let label = number_label(orig_index + 1);
