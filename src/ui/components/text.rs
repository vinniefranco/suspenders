//! The shared text + column primitives every render path funnels through: hard
//! line-splitting, greedy word-wrap, display-column truncation/clipping, the
//! column-bounded span push, and the small layout helpers. Split from the
//! components god module so the ONE column-arithmetic (measure==draw, ADR-0029)
//! lives in one place; ratatui types arrive via `use super::*`.
