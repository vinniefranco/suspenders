...
//! space-between, no powerline chrome. Split from the components god module by
//! rendering responsibility; the pure [`footer`] assembly (which items, in what
//! order) stays ratatui-free (ADR-0019) so the colocated tests assert it without
//! a frame, and [`render_footer`] is the thin painter over it.
...
impl Footer {
    ...
    pub(super) fn fit(mut self, width: usize, left_cells: usize) -> Footer {
    ...
    pub(super) fn cells(&self, left_cells: usize) -> usize {
...
pub(super) fn mcp_offline_label(offline: usize) -> Option<String> {
    if offline == 0 {
        return None;
    }
...
pub(super) fn footer_left_cells(left: FooterLeft) -> usize {
    match left {
...
pub(super) const FOOTER_DROP_TIER_COUNT: usize = 2;
...
pub(super) const FOOTER_MIN_GAP: usize = 1;
...
pub(super) const COST_SUB_CENT: f64 = 0.01;
...
pub(super) const COST_HIDDEN: f64 = 0.0;
