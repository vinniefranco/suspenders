...
/// it. The added/removed background TINT is resolved together with the marker
/// glyph by [`diff_side_display`].
...
/// The committed System A command's draw context (Parameter Object): the command
/// name, its status, the selector rows, and the active row index. Bundled so
/// [`dialog_draw`] stays a pure integration step with a readable signature.
...
fn dialog_draw(ctx: &DialogContext<'_>, inner_width: u16, theme: &Theme) -> PopupDraw {
    let title = selector_popup_title(ctx.command);
    let lines = match dialog_status_line(ctx.status, &title, theme) {
        Some(line) => vec![line],
        None => dialog_rows(ctx.rows, ctx.active, inner_width, theme),
    };
    PopupDraw {
        title,
        lines,
        scroll_active: Some(ctx.active),
...
fn render_composer_popup(
    ...
    let inner_width = area.width.saturating_sub(POPUP_BORDER_CHROME).max(1);
    let plan = match view {
        OverlayView::Menu {
            suggestions,
            active,
...
fn prepare_label(
    ...
    // Appends `...` to a slice string - used when truncating a label that overflows.
    let trail_elide = |s: String| -> String { format!("{}...", s) };
    ...
    let Some((m_start, raw_end)) = hit else {
        ...
        let before = if !is_expanded && long {
            trail_elide(slice(0, completion::MAX_WIDTH))
        } else {
            label.to_string()
        };
...
// Number of chars replaced by `...` at a clipped label boundary (qwen uses 3
// in both `elide_prefix` and `elide_suffix`).
const ELIDE_CHARS: usize = 3;

// Replaces the first `ELIDE_CHARS` chars of `s` with `...` (qwen `'...' +
// before.slice(3)`), or `...` when shorter.
fn elide_prefix(s: &str) -> String {
    let chars: Vec<char> = s.chars().collect();
    if chars.len() >= ELIDE_CHARS {
        format!("...{}", chars[ELIDE_CHARS..].iter().collect::<String>())
    } else {
        "...".to_string()
    }
}

// Replaces the last `ELIDE_CHARS` chars of `s` with `...` (qwen
// `after.slice(0, -3) + '...'`), or `...` when shorter.
fn elide_suffix(s: &str) -> String {
    let chars: Vec<char> = s.chars().collect();
    if chars.len() >= ELIDE_CHARS {
        format!(
            "{}...",
            chars[..chars.len() - ELIDE_CHARS]
                .iter()
                .collect::<String>()
        )
    } else {
...
// Formats a row number as `N.` (e.g. `1.`, `12.`) - the label used in every
// numbered list to size and render the number field.
fn num_dot(n: usize) -> String {
    format!("{}.", n)
}
...
fn dialog_rows(
    ...
    let num_field = num_dot(rows.len()).width();
    rows.iter()
        .enumerate()
        .map(|(i, row)| {
...
/// Columns the composer popup's rounded box border + inner padding consume: one
/// column each for the left border, left padding, right padding, and right border.
const POPUP_BORDER_CHROME: u16 = 4;
...
fn sticky_number_col(shown: &[(usize, &TodoItem)]) -> usize {
    shown
        .iter()
        .map(|(orig, _)| num_dot(orig + 1).chars().count())
        .max()
        .unwrap_or(2)
        + 1
}
...
fn sticky_row_spans(
    ...
    let label = num_dot(orig_index + 1);
...
/// Total row count of the bordered info panel: 1 top border + 4 content rows
/// + 1 bottom border, so it lines up beside the 6-row logo.
const HEADER_BOX_ROW_COUNT: usize = 6;
...
fn header_boxed_panel(view: &HeaderView<'_>, inner: usize, theme: &Theme) -> Vec<Line<'static>> {
    let border = border_style(theme);
    let mut rows: Vec<Line<'static>> = Vec::with_capacity(HEADER_BOX_ROW_COUNT);
...
fn prefixed_text_lines(
    ...
    let glyph_span = || Span::styled(format!("{glyph} "), prefix_style);
    let mut out = Vec::new();
    let mut first = true;
...
/// The marker glyph and background tint a diff line's [`DiffSide`] draws
/// (ADR-0008): both are side properties and are always queried together in
/// [`diff_code_row`], so a single match resolves both at once. The marker glyph
/// is two cells wide so code text aligns across sides; the tint is `None` for
/// context (untinted) and the Theme `*_bg` slot for added/removed.
fn diff_side_display(side: DiffSide, theme: &Theme) -> (&'static str, Option<Color>) {
    match side {
...
/// The line data for one diff code row (Parameter Object): the semantic side,
/// the pre-formatted gutter cell, the plain code text, and the optional syntect
/// colour fragments. Bundled so [`diff_code_row`] keeps a readable signature
/// while [`hunk_code_lines`] assembles each row from its loop variables.
struct DiffCodeLine<'a> {
    side: DiffSide,
    gutter: &'a str,
    text: &'a str,
...
fn diff_code_row(row: DiffCodeLine<'_>, width: usize, theme: &Theme) -> Line<'static> {
    let DiffCodeLine {
        side,
        gutter,
        text,
    ...
    let (marker, tint) = diff_side_display(side, theme);
    ...
    used = push_cols(&mut spans, marker, band(semantic), used, width);
...
#[cfg(test)]
#[path = "../../tests/unit/ui/components.rs"]
mod tests;
