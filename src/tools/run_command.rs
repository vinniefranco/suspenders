...
/// Time-unit conversion constants used by the sleep-duration parser.
const MILLIS_PER_SEC: f64 = 1_000.0;
const SECS_PER_MINUTE: f64 = 60.0;
const MINS_PER_HOUR: f64 = 60.0;
const HOURS_PER_DAY: f64 = 24.0;

/// Threshold below which a sleep duration is NOT blocked (in seconds).
const BLOCKED_SLEEP_MIN_SECS: f64 = 2.0;
...
async fn spawn_and_wait(
    ...
    // SAFETY: the `pre_exec` closure runs post-fork/pre-exec in the child; the
    // sole call is `setpgid(0, 0)`, which is async-signal-safe per POSIX, so
    // there is no re-entrancy hazard from the forked memory state.
    unsafe {
        cmd.pre_exec(|| {
...
fn has_top_level_trailing_background_operator(command: &str) -> bool {
    ...
    if crate::tool::path::preceding_backslash_count(&chars, amp_index) % 2 == 1 {
        return false;
    }
...
fn detect_blocked_sleep_pattern(command: &str) -> Option<String> {
    ...
    if secs < BLOCKED_SLEEP_MIN_SECS {
        return None;
    }
...
fn parse_sleep_duration_to_seconds(token: &str) -> Option<f64> {
    ...
    match unit.as_str() {
        ...
        "" | "s" => Some(value),
        "m" => Some(value * SECS_PER_MINUTE),
        "h" => Some(value * SECS_PER_MINUTE * MINS_PER_HOUR),
        "d" => Some(value * SECS_PER_MINUTE * MINS_PER_HOUR * HOURS_PER_DAY),
        _ => None,
    }
}
...
#[cfg(test)]
#[path = "../../tests/unit/tools/run_command.rs"]
mod tests;
