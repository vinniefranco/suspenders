...
/// Pure MIME token check (qwen `MIME_TYPE_RE` token grammar): a non-empty
/// string of ASCII alphanumerics or the allowed punctuation characters.
...
/// Validate the subtype portion of a MIME type: `subtree.subtype` with an
/// optional `+suffix`. Returns true when all parts are valid tokens.
fn is_valid_mime_subtype(rest: &str) -> bool {
...
fn is_valid_mime(key: &str) -> bool {
    match key.split_once('/') {
        Some((ty, rest)) => is_mime_token(ty) && is_valid_mime_subtype(rest),
        None => false,
    }
}
...
#[cfg(test)]
#[path = "../../../../tests/unit/tools/read_file/notebook/mod.rs"]
mod tests;
