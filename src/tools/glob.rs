...
impl Tool for Glob {
    ...
    async fn run(&self, input: &Value, ctx: &ToolCtx) -> Result<String, String> {
        ...
        with_path(path.unwrap_or("."), ctx, |abs| {
            ...
            Ok(search(abs, &root, &ctx.root, &query))
        })
    }
}

/// The glob search specification: the compiled regex plus the display strings
/// qwen includes in every header/message.
struct GlobQuery<'a> {
    regex: &'a Regex,
    pattern: &'a str,
    location: &'a str,
...
fn search(dir: &Path, rel_root: &Path, project_root: &Path, query: &GlobQuery<'_>) -> String {
    let pattern = query.pattern;
    let location = query.location;

    let mut matched: Vec<Entry> = walk_files(dir)
        .into_iter()
        .filter(|file| query.regex.is_match(&relative_to(file, rel_root)))
