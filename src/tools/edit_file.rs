...
//! `packages/core/src/tools/edit.ts` (`EditTool` / `EditToolInvocation`),
//! narrowed to Suspenders' shape. Its `editHelper.ts` normalization pipeline
//! lives in the [`crate::tools::edit_match`] leaf this tool consumes.
//!
//! Matching tries the exact literal `old_string` first, then falls back to
//! qwen's `normalizeEditStrings` pipeline ([`edit_match`]): character-level
...
impl Tool for EditFile {
    ...
    async fn run_rich(
        ...
        // its own diff (ADR-0007: diff rendering lives in the tool) and attaches
...
///
/// 1. `edit_match::edit_strings` - if the literal `old_string` is not present,
...
///    empty line);
/// 2. `edit_match::augment_old_string_for_deletion` - for a pure deletion,
///    extend the string to replace by a trailing newline when the file has one,
///    so the removal leaves no blank line;
...
fn edited_content(
    ...
    let edit_match::Normalized {
        old_string: mut final_old,
        new_string: final_new,
    } = edit_match::edit_strings(content, old_string, new_string);
    ...
    final_old = edit_match::augment_old_string_for_deletion(content, &final_old, &final_new);

    // (3) EXACT occurrence counting on the canonicalized string.
    let occurrences = edit_match::count_occurrences(content, &final_old);
    if occurrences == 0 {
