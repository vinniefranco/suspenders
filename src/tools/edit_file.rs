...
impl Tool for EditFile {
    ...
    async fn run(&self, input: &Value, ctx: &ToolCtx) -> Result<String, String> {
        ...
        apply_edit(&abs, &file_path, &spec, ctx)
    }
}

/// The edit specification (the model-supplied strings + replace flag), separate
/// from the filesystem path and execution context so `apply_edit` stays within
...
fn apply_edit(
    ...
    let is_new_file = spec.old_string.is_empty() && current.is_none();

    let new_content = match &current {
        // ---- new-file creation (empty old_string, file absent) ----
        None if spec.old_string.is_empty() => spec.new_string.to_string(),
        ...
        Some(content) => {
            ...
            edited_content(content, spec, file_path)?
        }
    };
...
fn edited_content(content: &str, spec: &EditSpec<'_>, file_path: &str) -> Result<String, String> {
    let EditSpec {
        old_string,
        new_string,
        replace_all,
...
mod normalize {
        ...
        if let Some(idx) = attempt_match(&lines, &pattern_lines) {
            return Some(MatchedSlice {
                slice: slice_from_lines(
                    ...
                    &SliceSpec {
                        start_line: idx,
                        line_count: pattern_lines.len(),
                        include_trailing_newline: ends_with_newline,
                    },
                ),
                removed_trailing_final_empty_line: false,
            });
    ...
    fn attempt_match(lines: &[&str], pattern: &[&str]) -> Option<usize> {
        // Three passes (per qwen): identity, trimEnd, then normalize+trimEnd.
        // The cast on the first element lets Rust infer the array type without
        // an explicit size literal.
        let passes = [
            (|v: &str| v.to_string()) as fn(&str) -> String,
    ...
    /// The slice parameters for `slice_from_lines` (qwen `sliceFromLines`):
    /// which lines to reconstruct and whether to include the trailing newline.
    struct SliceSpec {
        start_line: usize,
        line_count: usize,
    ...
    fn slice_from_lines(text: &str, offsets: &[usize], lines: &[&str], spec: &SliceSpec) -> String {
        if spec.line_count == 0 {
            return if spec.include_trailing_newline {
                "\n".to_string()
            } else {
                String::new()
            };
        }
        let start_index = offsets.get(spec.start_line).copied().unwrap_or(0);
        let last_line_index = spec.start_line + spec.line_count - 1;
        ...
        if spec.include_trailing_newline {
            if let Some(next_line_start) = offsets.get(spec.start_line + spec.line_count).copied() {
...
#[cfg(test)]
#[path = "../../tests/unit/tools/edit_file.rs"]
mod tests;
