...
/// Numerator in the `window * CHARS_NUMER / CHARS_DENOM` formula: 3.5
/// chars/token at a 1/16 share = 7/32.
const CHARS_NUMER: u64 = 7;
/// Denominator in the `window * CHARS_NUMER / CHARS_DENOM` formula.
const CHARS_DENOM: u64 = 32;
...
pub fn cap_for(context_budget: u64, max_tokens_reserve: u64) -> usize {
    ...
    // window_tokens * 3.5 chars/token, a sixteenth of it: window * CHARS_NUMER / CHARS_DENOM.
    ((window_tokens * CHARS_NUMER / CHARS_DENOM) as usize).max(FLOOR_CHARS)
}
...
#[cfg(test)]
#[path = "../../tests/unit/tools/shaping.rs"]
mod tests;
