...
Launch worker agents **in parallel** by invoking the `agent` tool (the Agent tool) multiple times in a **SINGLE message**.

**Note**: The `agent` tool in allowedTools is the Agent tool used to spawn worker agents.
