...
async fn execute_tool<D: RunDeps>(
    ...
    // Caller filters with `is_tool_use()` before dispatching here; the `_`
    // arm is provably unreachable. `expect` documents the caller invariant
    // so any future code-path change surfaces immediately.
    let ContentBlock::ToolUse { id, name, input } = block else {
        unreachable!("execute_tool caller guarantees a ToolUse block");
    };
