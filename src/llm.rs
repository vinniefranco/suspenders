...
/// Wire-token table for [`ToolCallStyle`]: each entry is `(token, variant)`.
/// Both [`as_str`](ToolCallStyle::as_str) and [`parse`](ToolCallStyle::parse)
/// use this single source of truth so the encoding/decoding pair stays in sync.
const TOOL_CALL_STYLE_TOKENS: &[(&str, ToolCallStyle)] = &[
    ("auto", ToolCallStyle::Auto),
    ("structured", ToolCallStyle::Structured),
    ("text", ToolCallStyle::Text),
];
...
impl LlmRequest {
    /// Constructs a request with the required fields; all optional fields take
    /// their defaults (no temperature, no sampling params, standard Tool Call
    /// style). Direct struct-update syntax (`LlmRequest { field, ..Default::default() }`)
    /// is also valid when a caller only needs to set a subset.
...
/// Extracts a text body from a successful models-listing HTTP response and
/// passes it to [`models_from_body`]. Both adapters' `list_models` finish
/// identically: the response check, body extraction, and parse are shared here
/// rather than duplicated per protocol. Connection errors and the sent-request
/// failure are protocol-specific and stay in each adapter.
pub(crate) async fn extract_model_list(resp: reqwest::Response) -> Result<Vec<String>, String> {
