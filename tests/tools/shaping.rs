...
fn st(policy: CutPolicy, input: &Value, content: &str, cap: usize) -> String {
    let out = shape(policy, input, vec![ResultBlock::text(content)], cap);
    match out.as_slice() {
...
fn content_within_the_cap_passes_through_untouched() {
    assert_eq!(
        st(CutPolicy::HeadWithResume, &json!({}), "short", 100),
        "short"
    );
    assert_eq!(st(CutPolicy::HeadTail, &json!({}), "short", 100), "short");
}
...
fn content_exactly_at_the_cap_passes_through_untouched() {
    ...
    assert_eq!(
        st(CutPolicy::HeadWithResume, &json!({}), &content, 100),
        content
    );
}
...
fn head_only_shaping_keeps_first_cap_chars_and_appends_marker() {
    ...
    assert_eq!(
        st(CutPolicy::Head, &json!({}), &content, 100),
        format!(
...
#[test]
fn head_with_resume_shaping_cuts_at_line_boundary_and_names_resume() {
    ...
    let shaped = st(CutPolicy::HeadWithResume, &json!({}), &content, 100);
...
#[test]
fn head_with_resume_shaping_keeps_line_numbers_file_absolute_via_offset() {
    let content = (1..=30)
        .map(|i| format!("line-{i:04}"))
        .collect::<Vec<_>>()
        .join("\n");
...
#[test]
fn head_with_resume_shaping_treats_a_missing_or_non_numeric_offset_as_zero() {
    let content = (1..=30)
        .map(|i| format!("line-{i:04}"))
        .collect::<Vec<_>>()
        .join("\n");
...
#[test]
fn the_head_policy_ignores_an_offset_in_the_input() {
    let content = "a".repeat(250);
    assert_eq!(
        st(CutPolicy::Head, &json!({"offset": 20}), &content, 100),
        format!(
            "{}\n[truncated: output is 250 chars, showing the first 100]",
            "a".repeat(100)
        )
...
#[test]
fn head_with_resume_shaping_ignores_trailing_newlines_empty_split() {
    let content = (1..=30)
        .map(|i| format!("line-{i:04}\n"))
        .collect::<Vec<_>>()
        .join("");
...
#[test]
fn head_with_resume_first_line_wider_than_cap_falls_back_to_head_cut() {
    let content = "a".repeat(250);
    assert_eq!(
        st(CutPolicy::HeadWithResume, &json!({}), &content, 100),
        format!(
...
#[test]
fn head_tail_shaping_keeps_head_and_tail_with_omission_marker() {
    let content = format!("HEAD{}TAIL", "x".repeat(500));
    let shaped = st(CutPolicy::HeadTail, &json!({}), &content, 100);
