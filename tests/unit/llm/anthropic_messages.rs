...
async fn assembles_tool_use_input_from_partial_json_split_across_events() {
    ...
    assert_eq!(
        result.content,
        vec![
            make_tool_use("toolu_1", "read_file", json!({ "path": "x" })),
            make_tool_use("toolu_2", "list_directory", json!({})),
        ]
    );
...
async fn omitted_block_indexes_do_not_collapse() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            make_tool_use("t1", "list_directory", json!({ "path": "." })),
        ]
    );
}
...
async fn sends_system_max_tokens_stream_tools_and_wire_format_messages() {
    ...
    let messages = vec![
        ...
        Message::assistant(vec![
            ...
            make_tool_use("toolu_1", "read_file", json!({ "path": "mix.exs" })),
        ]),
