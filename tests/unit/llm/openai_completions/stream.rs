...
fn assembles_tool_call_from_fragments_keyed_by_index() {
    ...
    assert_eq!(
        r.content,
        vec![make_tool_use("call_1", "read_file", json!({ "path": "x" }))]
    );
...
fn parallel_tool_calls_interleave_without_collapsing() {
    ...
    assert_eq!(
        r.content,
        vec![
            make_tool_use("c1", "read_file", json!({ "path": "a" })),
            make_tool_use("c2", "list_directory", json!({})),
        ]
    );
}
...
fn text_precedes_tool_calls_in_final_content() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            make_tool_use("c1", "list_directory", json!({})),
        ]
    );
}
...
fn a_fragment_without_index_targets_the_last_opened_call() {
    ...
    assert_eq!(
        r.content,
        vec![make_tool_use("c1", "read_file", json!({ "path": "." }))]
    );
}
...
fn empty_accumulated_arguments_become_an_empty_map() {
    ...
    assert_eq!(
        r.content,
        vec![make_tool_use("c1", "list_directory", json!({}))]
    );
}
...
fn snapshot_shows_open_tool_calls_with_input_null() {
    ...
    assert_eq!(
        s.snapshot(),
        vec![make_tool_use("c1", "read_file", Value::Null)]
    );
}
...
fn a_structured_tool_call_wins_over_coincident_text_markup() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            make_tool_use("call_1", "read_file", json!({ "path": "x" })),
        ]
    );
    assert_eq!(r.stop_reason, StopReason::ToolUse);
}
