...
// An `assistant_blocks` entry with unknown Provenance (production stamps the
// struct form directly; these round-trip/parse tests only need the unstamped
// shape).
fn assistant_blocks(blocks: Vec<ContentBlock>) -> Entry {
...
fn a_settled_run_folds_back_into_the_exact_conversation_shape() {
    ...
    log.append(assistant_blocks(vec![
        text("Let me look."),
    ...
    log.append(assistant_blocks(vec![text("Two files.")]));
...
fn a_mixed_batch_keeps_answered_tool_calls_and_drops_unanswered_ones() {
    ...
    log.append(assistant_blocks(vec![text("done")]));
    log.append(Entry::Settled {
        outcome: Settled::Completed,
        stop_reason: StopReason::EndTurn,
        reason: None,
...
fn plan_restores_the_last_logged_plan_which_never_enters_the_folded_messages() {
    ...
    log.append(assistant_blocks(vec![text("planned")]));
    log.append(Entry::Plan(
        "Goal: A. 1. read [x] 2. edit [x] 3. verify [ ]".into(),
    ));
    log.append(assistant_blocks(vec![text("done step 2")]));
    log.append(Entry::Settled {
        outcome: Settled::Completed,
        stop_reason: StopReason::EndTurn,
        reason: None,
...
fn a_log_with_no_plan_entry_restores_a_nil_plan() {
    ...
    log.append(assistant_blocks(vec![text("hello")]));
    log.append(Entry::Settled {
        outcome: Settled::Completed,
        stop_reason: StopReason::EndTurn,
        reason: None,
...
fn an_adr_0009_truncated_batch_folds_back_intact() {
    ...
    log.append(Entry::UserText("go".into()));
    log.append(assistant_blocks(vec![tool_use(
        "t1",
        "write_file",
        json!({"path": "a"}),
    )]));
    ...
    log.append(assistant_blocks(vec![text("re-issued")]));
    log.append(Entry::Settled {
        outcome: Settled::Completed,
        stop_reason: StopReason::EndTurn,
        reason: None,
...
fn a_failed_settlement_carries_its_reason_string_forensically_the_fold_ignores_it() {
    ...
    log.append(Entry::UserText("go".into()));
    log.append(assistant_blocks(vec![text("partial")]));
    log.append(Entry::Settled {
...
fn a_compaction_fold_discards_raw_entries_before_the_compacted_marker() {
    ...
    log.append(assistant_blocks(vec![text("old response")]));
    log.append(Entry::UserText("turn 2".into()));
    log.append(assistant_blocks(vec![text("compacted response")]));
    ...
    log.append(assistant_blocks(vec![text("new response")]));
    log.append(Entry::Settled {
        outcome: Settled::Completed,
        stop_reason: StopReason::EndTurn,
        reason: None,
...
async fn a_live_compaction_and_its_logged_fold_reconstruct_byte_identical_messages() {
    ...
    for i in (1..=5).rev() {
        ...
        log.append(assistant_blocks(vec![text(&body)]));
    }
    ...
    for msg in &compacted.messages[1..] {
        match msg.role {
            ...
            Role::Assistant => {
                log.append(assistant_blocks(msg.content.clone()));
            }
        }
    }
    log.append(Entry::Settled {
...
fn a_retry_entry_is_silent_to_the_folded_conversation() {
    ...
    log.append(assistant_blocks(vec![text("re-drawn answer")]));
    log.append(Entry::Settled {
        outcome: Settled::Completed,
        stop_reason: StopReason::EndTurn,
        reason: None,
