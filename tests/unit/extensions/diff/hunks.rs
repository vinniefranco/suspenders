...
// Local helper: renders hunks to classic unified-diff text (@@-headers + +/-/space
// prefixes), capped at max_lines with a trailing elision note when cut. Covers the
// tests that previously exercised the now-deleted production `to_unified`.
fn to_unified(hunks: &[Hunk], max_lines: usize) -> String {
