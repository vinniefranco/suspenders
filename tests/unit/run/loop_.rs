...
async fn a_multi_tool_pass_checkpoints_once_with_the_whole_answered_batch() {
    ...
    let two_tool_pass = Response {
        content: vec![
            make_tool_use("tu_1", "list_directory", json!({"path": "."})),
            make_tool_use("tu_2", "list_directory", json!({"path": "."})),
        ],
...
async fn errored_response_settles_failed_keeping_partial_text() {
    ...
    let errored = Response {
        content: vec![
            ...
            make_tool_use("t1", "grep_search", json!({"pattern": "x"})),
        ],
...
async fn reissued_call_after_truncation_executes_not_duplicate() {
    ...
    let deps = deps_for(
        ...
        vec![
            just(Response {
                content: vec![make_tool_use("t1", "write_file", input.clone())],
                stop_reason: StopReason::MaxTokens,
                usage: Usage::default(),
                error: None,
            }),
...
async fn tool_use_with_a_non_tool_use_stop_reason_still_continues() {
    ...
    let end_turn_with_tool = Response {
        content: vec![make_tool_use("t1", "list_directory", json!({"path": dir}))],
        stop_reason: StopReason::EndTurn,
        usage: Usage::default(),
        error: None,
    };
