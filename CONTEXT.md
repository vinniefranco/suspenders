...
**At Mention**:
A `@<path>` reference in the Composer draft (qwen's `@path`, ported as `useAtCompletion` file completion). Today suspenders inserts it as plain text and the model resolves it by calling read_file; the faithful qwen behavior is that it is EXPANDED at submit (see **At Expansion**).
_Avoid_: file reference, path token

**At Expansion**:
The submit-time pass (qwen's `atCommandProcessor`) that reads every **At Mention**'s file and turns it into content on the user Message before the Run starts: a text file inlines as text, an image or PDF becomes an `Image`/`Document` content block (base64, reusing read_file's encoder and mime/magic-byte detection). This is the spine suspenders never ported - the reason a dragged image is inert today. Media reaches the model as first-class user content - never a synthesized Tool Call, never a fabricated Tool Result.
_Avoid_: at-command (qwen's file name; suspenders names the act), interpolation

**Attachment**:
Media the user brings to a prompt that has no natural path to mention - qwen's word for the transient state of a clipboard image (InputPrompt "attachment state for clipboard images"). Raw image bytes from the system clipboard are written to a temp file (qwen's `saveClipboardImage`, via `wl-paste`/`xclip` on Linux), staged as an Attachment, and converted to an **At Mention** of that temp file at submit - so a clipboard image rejoins the one **At Expansion** spine rather than being a second content path. A dragged or pasted file path is NOT an Attachment: it becomes an **At Mention** directly in the draft, never staged.
_Avoid_: upload (there is no server; nothing is uploaded), enclosure (considered and rejected 2026-08-03), Artifact (that is display-side tool data, never model-facing - the opposite direction), a general "any pinned media" meaning (an on-disk path is an **At Mention**, not an Attachment)
...
_Avoid_: Presenter output (the retired role is gone; the Tool owns the Artifact now), attachment (that is now the user-supplied media term - see **Attachment**; an Artifact is display-side tool data, never model-facing, the opposite direction)
