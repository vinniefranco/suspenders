...
_Avoid_: input line (it is not a line; drafts may span many), prompt (that's what a submitted draft becomes), selector modal (an overlay is a Composer state; nothing in the UI is a centered modal - an Approval prompt is an inline block in the Pending Region that merely captures keys)
