...
Config resolves once at launch, in precedence order: built-in defaults → `config.json` → `SUSPENDERS_*` environment.
...
| `max_tokens` | — | output cap; the eviction budget derives from it |
| `temperature` | none | sampling temperature, omitted if unset |
