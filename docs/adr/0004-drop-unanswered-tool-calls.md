...
> Amended by ADR-0009: the truncation path now answers Tool Calls with an error Tool Result instead of dropping them. The drop rule below applies where no retry Pass exists to answer the call: Cancellation, and responses that ended in a stream error (the Run settles as failed).
...
- **Fail the Run.** Loud, but discards good text the model produced before the cut, and punishes the user for a tuning problem (max_tokens) or their own Escape key.

Consequence: code in the Run/Agent layer deliberately deletes blocks the model emitted. That is not a bug; do not "fix" it by passing them through.
