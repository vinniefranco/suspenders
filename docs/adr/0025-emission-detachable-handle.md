# Event emission is a detachable Emitter handle, not a RunDeps method

Event emission is an owned `Emitter` handle - `Emitter::new(impl FnMut(Event) + Send)`, one `emit(Event)` method - that the Run loop obtains once from `RunDeps::emitter()` and carries beside the deps. Every other effect stays a trait method (ADR-0011); emission alone moves out.
...
- **A second generic `Emit` parameter on `run` (`run<D: RunDeps, E: Emit>`).** The same borrow split, but a second type parameter infects every call site and helper signature, and formally breaks ADR-0011's shape - one trait that **is** every effect the loop needs.
