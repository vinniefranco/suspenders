...
makes Tool Calls every Pass until the Run Limit is never verify-nudged,
...
A verified or write-free Run is untouched: ordinary wrap-up warning, full
Tool list, ADR-0015 Endgame as before.

## Consequences

- The Endgame for an unverified Run is warning-with-verify-prompt at
...
- A model that answers the Verification Pass with other Tools anyway gets
  ...
  owes.
- One editing Pass per unverified capped Run is traded for a
  verification. Driving evidence says that Pass was not saving those
  Runs - both failures spent their late Passes on doc-coherence edits
