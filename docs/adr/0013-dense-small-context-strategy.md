...
2. **Prefill is the latency budget.** Every cache-invalidating change
   (Eviction wave, Compaction) re-processes the prefix on local hardware.
   Invalidating a 50k prefix is an annoyance; invalidating a 120k prefix
   is a coffee break per Pass.
...
summarizes a large coherent span, and long append-only stretches between
them keep the server prompt cache warm.
...
- Companion mechanisms carry the strategy: the Plan/Anchor (task state
  pinned near the tail, where a small model actually attends) and the
  Scout (exploration in a disposable Conversation so search never enters
  the main window). See CONTEXT.md for the language.
- Thinking stays enabled at every call site (main loop, Scout,
  Compaction), uniform behavior over server-dependent toggles; the
  reserve and the Scout's Pass cap are sized with that cost in mind.
  (Superseded for the Scout by ADR-0014: live probing showed thinking is
  fatal for Scouts on Qwen3.5, so Scouts default to no-think.)
