...
We ship one binary. The terminal UI (ratatui, ADR-0001) is walled into a single `ui` module, and `--headless` selects a stdout event-subscriber runner instead of launching the UI. Everything below the UI — the Agent, Turn, Tools, LLM, and Session — is UI-free and event-driven, so tests and the headless runner drive the Agent purely through Commands plus the Event broadcast and never touch a TTY.
