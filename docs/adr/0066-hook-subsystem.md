...
This is possible because hooks carry only the generic lifecycle-interception concern. Tool-specific behavior lives in the Tool that owns it: the diff, todo, and `run_command` output behaviors belong to their respective Tools, and each renders its own rich result. The condense/summarize behavior belongs to the compaction service (ADR-0012), which owns the context-shrinking decision. That placement is recorded in ADR-0007. With those concerns housed where they belong, hooks are left as the ONLY generic lifecycle layer, and each hook site stays explicit and easy to reason about.
