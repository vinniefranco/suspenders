...
not a conclusion. 3 of 4 long Turns ended this way; the one success
...
When the Pass reaches the Turn Limit, the request carries an empty Tool
...
A Turn that concludes this way ends on the end-turn stop reason with the
model's own status statement as its last message - strictly better than
the marker for the user, the Session Log, and any Turn that follows. The
...
run and the Turn closes exactly as before this ADR).
...
Conversation (kept, it would prime later Turns to emit more of it). The
...
the no-think rescue all already guard on the Pass being below the Turn
Limit, so none of them can loop past the final Pass. An empty final
response closes on the empty marker as before.

## Consequences

- Capped Turns end with a real answer or an explicit "what remains undone"
  statement instead of silence; turn-limit settlements become rare and
  genuinely mean "the model would not stop calling Tools".
- One Tool batch per capped Turn is traded for the conclusion. Driving
  evidence says that batch was not saving those Turns (runs 3-4: the
