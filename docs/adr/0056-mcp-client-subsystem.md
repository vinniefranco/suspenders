...
We use the `rmcp` crate (v3.0.1) for the MCP wire protocol, with two transports: `TokioChildProcess` (stdio - spawn a command with args/env/cwd) and the streamable-HTTP client (a URL plus static headers). Every `rmcp` import lives in ONE file, `src/mcp/manager.rs`: the transport construction, the `().serve(transport).await` handshake, `list_tools`, and the `CallToolResult` decode. Nothing else in the crate touches `rmcp`.
...
- **SSE and websocket transports** - stdio and streamable-HTTP only.
