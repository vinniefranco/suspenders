...
The module tree mirrors the domain's decomposition 1:1 with snake_case names - for example `run/loop_`, `conversation`, `tools/edit_file`, `extensions/diff`, `run/governor` (nudges are governors now), `run/governor/endgame`, `run/settlement` - and each unit of behaviour has one colocated test module.
